...
export class BomViewComponent implements OnInit {
  ...
  loadData() {
    this.merchService.getAllBOMItems().subscribe(data => this.bomItems = data);
  ...
  onSubmit() {
    if (this.bomForm.valid) {
      ...
      const bomItem: BOMItem = {
        ...
        unit: formData.unit,
        consumption: 0, // Fallback for old model requirement
        wastagePercent: 0,
        totalRequirement: 0,
      ...
      this.merchService.createBOMItem(bomItem).subscribe(() => {
        this.notify.success('BOM View added successfully');
